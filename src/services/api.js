...
export const toggleHot = (requestId) =>
  api.patch('/requests/hot', null, { params: { requestId } })
...
// ── VOICE NOTES ──────────────────────────────────────────────────────────
export const getVoiceNotes = (requestId) =>
  api.get('/voice-notes', { params: { requestId } })

export const addVoiceNote = (requestId, audioFile, noteText, durationSecs, recipientName) => {
  const formData = new FormData()
  formData.append('audio', audioFile)
  if (noteText) formData.append('noteText', noteText)
  if (durationSecs) formData.append('durationSecs', durationSecs)
  if (recipientName) formData.append('recipientName', recipientName)
  return api.post('/voice-notes', formData, {
    params: { requestId },
    headers: { 'Content-Type': 'multipart/form-data' },
  })
}
