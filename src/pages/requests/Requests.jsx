  ...
  return (
    <div className="space-y-4 fade-in">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Manage and track ward member requests</p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 sm:flex-none px-5 py-2 rounded-xl text-white text-sm font-medium"
              style={{ backgroundColor: '#0D47A1' }}
            >
              Search
            </button>
            <button
              onClick={() => { setKeyword(''); setStatus(''); setPage(0) }}
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">File No.</th>
                  </tr>
                </thead>
                  ...
                  {requests.map((req) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        {req.sections && (
                          <p className="text-xs text-gray-400 mt-0.5">{req.sections}</p>
                        )}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-600">Ward {req.wardNumber}</td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">{req.fileNumber || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="md:hidden divide-y divide-gray-100">
              {requests.map((req) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 mt-2 ml-4 text-xs text-gray-400">
                    <span>Ward {req.wardNumber}</span>
                    <span>•</span>
                    <span>{formatDate(req.createdAt)}</span>
                    {req.fileNumber && <><span>•</span><span>📁 {req.fileNumber}</span></>}
                  </div>
                </div>
              ))}
            </div>
